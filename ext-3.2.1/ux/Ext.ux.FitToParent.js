...
Ext.ux.FitToParent = Ext.extend(Object, {
	...
	/**
	 * init
	 * @param {Ext.Panel} component
	 */
	init: function (component) {
		this.component = component;
		var onRenderFn = function onRenderFn(comp) {
				this.parent = Ext.get(this.parent || comp.getPositionEl().dom.parentNode);
				if (comp.doLayout) {
					comp.monitorResize = true;
					comp.doLayout = comp.doLayout.createInterceptor(this.fitSize, this);
				} else {
					this.fitSize();
					Ext.EventManager.onWindowResize(this.fitSize, this);
				}
			};
		component.on('render', onRenderFn, this, {
			single: true
		});
	},
	/**
	...
	fitSize: function () {
		...
		var width = this.fitWidth ? size.width - pos[0] - this.offsets[0] : undefined;
		var height = this.fitHeight ? size.height - pos[1] - this.offsets[1] : undefined;
